...
thumbnail: /blog/thumbnails/my-post-title.svg # optional, generated by default
---
```

`category` must be one of the four slugs above (the build fails loudly otherwise). `metaTitle` and `metaDescription` override the browser, OpenGraph, Twitter, and JSON-LD metadata. At most one post should be `featured`; if none is, the newest post takes the hero card.
